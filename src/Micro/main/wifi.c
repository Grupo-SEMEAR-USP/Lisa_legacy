...
static const char *http_log_tag = "HTTP_CLIENT";
...
static void wifi_event_handler(
	...
	if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
		ESP_LOGI(http_log_tag, "Connecting to AP...");
		esp_wifi_connect();
	} else if (
		...
		if (s_retry_num < MAX_FAILURES) {
			ESP_LOGI(http_log_tag, "Reconnecting to AP...");
			esp_wifi_connect();
			s_retry_num++;
		} else {
...
static void ip_event_handler(
	...
	if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        ESP_LOGI(http_log_tag, "STA IP: " IPSTR, IP2STR(&event->ip_info.ip));
        s_retry_num = 0;
...
static esp_err_t http_event_handler(esp_http_client_event_t *evt) {
    ...
    switch (evt->event_id) {
        case HTTP_EVENT_ERROR:
            ESP_LOGD(http_log_tag, "HTTP_EVENT_ERROR");
            break;
        case HTTP_EVENT_ON_CONNECTED:
            ESP_LOGD(http_log_tag, "HTTP_EVENT_ON_CONNECTED");
            break;
        case HTTP_EVENT_HEADER_SENT:
            ESP_LOGD(http_log_tag, "HTTP_EVENT_HEADER_SENT");
            break;
        case HTTP_EVENT_ON_HEADER:
            ESP_LOGD(
                http_log_tag, "HTTP_EVENT_ON_HEADER, key=%s, value=%s", 
                evt->header_key, evt->header_value
            );
            break;
        case HTTP_EVENT_ON_DATA:
            ESP_LOGD(http_log_tag, "HTTP_EVENT_ON_DATA, len=%d", 
                evt->data_len
            );
            /*
        ...
        case HTTP_EVENT_ON_FINISH:
            ESP_LOGD(http_log_tag, "HTTP_EVENT_ON_FINISH");
            if (output_buffer != NULL) {
                /*
                 * Response is accumulated in output_buffer. 
                 * Uncomment the below line to print the accumulated response
                 */
                //ESP_LOG_BUFFER_HEX(http_log_tag, output_buffer, output_len);
                free(output_buffer);
                output_buffer = NULL;
            }
            output_len = 0;
        ...
        case HTTP_EVENT_DISCONNECTED:
            ESP_LOGI(http_log_tag, "HTTP_EVENT_DISCONNECTED");
            int mbedtls_err = 0;
            ...
            if (err != 0) {
                ESP_LOGI(http_log_tag, "Last esp error code: 0x%x", err);
                ESP_LOGI(http_log_tag, "Last mbedtls failure: 0x%x", 
                    mbedtls_err
                );
            }
            if (output_buffer != NULL) {
...
esp_err_t connect_wifi(void) {
    ...
    ESP_LOGI(http_log_tag, "STA initialization complete");
    ...
    if (bits & WIFI_SUCCESS) {
        ESP_LOGI(http_log_tag, "Connected to ap");
        status = WIFI_SUCCESS;
    } else if (bits & WIFI_FAILURE) {
        ESP_LOGI(http_log_tag, "Failed to connect to ap");
        status = WIFI_FAILURE;
    } else {
        ESP_LOGE(http_log_tag, "UNEXPECTED EVENT");
        status = WIFI_FAILURE;
    }
...
void http_rest_with_url(void) {
    ...
    if (err == ESP_OK) {
        ESP_LOGI(http_log_tag, "HTTP GET Status = %d, content_length = %d",
            esp_http_client_get_status_code(client),
            esp_http_client_get_content_length(client)
        );
    } else {
        ESP_LOGE(http_log_tag, "HTTP GET request failed: %s", 
            esp_err_to_name(err)
        );
    }
    ESP_LOGI(http_log_tag, "%s", local_response_buffer);
